...
export class AdminAreaComponent implements OnInit {
  ...
  image:string =""
  offerprice: any;
  constructor(private modalService:ModalService,private cartService: CartServiceService,private authService:AuthService, private router:Router) { }
      ...
      this.cartService.addImagePopup.subscribe((resData) => {
        if(resData == false){
          ...
          this.modalService.image.subscribe((res:any)=> {
            this.image = res
              axios.get("https://perlarest.vinoitalia.gr/php-auth-api/updateSingleImage.php/?id=11&mtrl="+resData+"&image="+this.image)
                .then(res=> {
      ...
      this.cartService.openOffer.subscribe((resData) => {
        ...
        this.modalService.offer.subscribe((res:number)=> {
          ...
          if(+res === 0){
            ...
            this.offerprice = +resData.wholesale - ((+resData.wholesale * +res) / 100)
