...
export class CartComponent implements OnInit, OnDestroy {
    ...
    if (items) {
      for (let product of prod.children) {
        if (product.children[0].nodeName != 'BUTTON') {
          ...
          inputsArray.push(
            product.children[1].children[0].children[0].children[0].children[0]
              .children[1]
          );
        }
      }
  ...
  handleCheckout(btn: any) {
    if (!btn.classList.contains('loading')) {
      ...
      axios
        .post('https://perlarest.vinoitalia.gr/php-auth-api/updateStock.php', {
          method: 'STOCKUPDATE',
        })
        .then((resData) => {
  ...
  handleContinueShopping() {
    let currentCategory = JSON.parse(
